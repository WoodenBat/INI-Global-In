...
public class BoardController {
 ...
 // 게시글 삭제
    ...
    @ResponseBody
    public ResponseEntity<?> toggleLike(@PathVariable int boardId, HttpSession session) {
        String userId = (String) session.getAttribute("loginUser");
        if (userId == null) {
    ...
    public String viewBoardWithLike(@PathVariable("id") int boardId, Model model, HttpSession session) {
        boardService.incrementViews(boardId);
        BoardDetailDTO board = boardService.getPostById(boardId);
        if (board == null) {
            return "redirect:/board/list";
